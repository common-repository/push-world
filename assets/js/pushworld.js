...
jQuery(document).ready(function ($) {
	...
	var $checkProtocol = $('input[name="sunrise[protocol]"]');
	$checkProtocol.on('change',function () {
		triggerFileChecker($(this));
	});

	$('.pw-btn__check').on('click', function () {
		var file = $(this).data('check'),
			url = window.location.origin + '/' + file;

		checkFiles(url, $(this));
	});

	function checkFiles(url, $this) {
		$.ajax({
			type: 'HEAD',
			url: url,
			complete: function (xhr){
				if (xhr.status == 404){
					$this.closest('td').removeClass('pw-exist').addClass('pw-noexist');
				} else {
					$this.closest('td').removeClass('pw-noexist').addClass('pw-exist');
				}
			}
		});
	}
	...
	function triggerFileChecker($this) {
		var checkFiles = $('.pw-check-file');
		if ($this.val() === 'https') {
			checkFiles.removeClass('pw-hidden');
		} else {
			checkFiles.addClass('pw-hidden');
		}
	}
