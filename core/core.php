...
if ( ! class_exists( 'Sunrise7' ) ) {
	...
	class Sunrise7 {
		...
		public function register() {
			if ( isset( $this->config['menus'] ) && count( $this->config['menus'] ) ) {
				foreach ( $this->config['menus'] as $menu ) {
					add_menu_page( $menu['page_title'], $menu['menu_title'], $menu['capability'], $menu['slug'],
						array( &$this, 'render' ), $menu['icon_url'], $menu['position'] );
				}
			}
			if ( isset( $this->config['pages'] ) && count( $this->config['pages'] ) ) {
				foreach ( $this->config['pages'] as $page ) {
					add_submenu_page( $page['parent_slug'], $page['page_title'], $page['menu_title'],
						$page['capability'], $page['slug'], array( &$this, 'render' ) );
				}
			}
		}
		...
		public function add_submenu( $args ) {
			...
			// Define page url
			if ( ! $args['url'] ) {
				if ( strpos( $args['parent_slug'], '.php' ) !== false && strpos( $args['parent_slug'],
						'?' ) !== false
				) {
			...
			// Save options to global config
			if ( is_array( $args['options'] ) && count( $args['options'] ) && ! in_array( $args['slug'],
					array_keys( (array) $this->config['menus'] ) )
			) {
				foreach ( $args['options'] as $option ) {
					$this->config['options'][] = $option;
				}
		...
		public function render() {
			...
			echo '<div id="sunrise-settings" class="wrap b-card b-card--main">';
			echo call_user_func( array( $this->config['views_class'], 'options_page_tabs' ), $options, $this->config );
			echo call_user_func( array( $this->config['views_class'], 'options_page_notices' ), $options,
				$this->config );
		...
		public function assets() {
			// Register styles
			wp_register_style( 'sunrise-' . $this->config['version'],
				plugins_url( $this->config['css'], $this->config['file'] ) . '/assets/css/pushworld.css', false,
				$this->config['version'], 'all' );
			// Register scripts
			wp_register_script( 'sunrise-' . $this->config['version'],
				plugins_url( $this->config['js'], $this->config['file'] ) . '/assets/js/pushworld.js',
				array( 'jquery', 'jquery-form' ), $this->config['version'], true );
			// Add some l10n to JS
		...
		public function enqueue() {
			...
			// Enqueue styles
			foreach ( array( 'farbtastic', 'sunrise-' . $this->config['version'] ) as $style ) {
		...
		public function submit() {
			...
			// Check nonce
			if ( empty( $_REQUEST['sunrise_nonce'] ) || ! wp_verify_nonce( $_REQUEST['sunrise_nonce'], 'sunrise' ) ) {
				return;
			}
			// Check page
			if ( ! $this->is_sunrise() ) {
		...
		public function get_page() {
			...
			if ( in_array( $slug, array_keys( (array) $this->config['menus'] ) ) ) {
				...
				if ( in_array( $slug, array_keys( (array) $this->config['pages'] ) ) ) {
		...
		public function get_page_options() {
			...
			if ( isset( $page['options'] ) && is_array( $page['options'] ) ) // Loop through page options
			{
...
if ( ! class_exists( 'Sunrise7_Views' ) ) {
	...
	class Sunrise7_Views {
		...
		public static function notice( $msg = '', $class = '' ) {
			return '<div class="sunrise-notice b-notification ' . $class . '"><p>' . $msg . '</p></div>';
		}

		public static function type_opentab( $field, $config ) {
			return '<div class="sunrise-pane"><h3 class="hide-if-js sunrise-no-js-tab">' . $field['name'] . '</h3><div class="form-table b-card__content">';
		}
		...
		public static function type_closetab( $field, $config ) {
			...
			$return[] = '</div>';
			if ( $field['actions'] ) {
				$return[] = '<div class="sunrise-actions-bar"><input type="submit" value="' . __( 'Save changes',
						$config['textdomain'] ) . '" class="sunrise-submit b-button b-button--blue" /><span class="sunrise-spin"><img src="' . admin_url( 'images/wpspin_light.gif' ) . '" alt="" /> ' . __( 'Saving',
						$config['textdomain'] ) . '&hellip;</span><span class="sunrise-success-tip"><img src="' . admin_url( 'images/yes.png' ) . '" alt="" /> ' . __( 'Saved',
						$config['textdomain'] ) . '</span><a href="' . $_SERVER["REQUEST_URI"] . '&amp;sunrise_action=reset&amp;sunrise_nonce=' . wp_create_nonce( 'sunrise' ) . '" class="sunrise-reset b-button alignright" title="' . esc_attr( __( 'This action will delete all your settings. Are you sure? This action cannot be undone!',
						$config['textdomain'] ) ) . '">' . __( 'Restore default settings',
						$config['textdomain'] ) . '</a></div>';
			}
			$return[] = '</div>';
		...
		public static function type_text( $field, $config ) {
			...
			return '<div class="b-content__row">
						<div class="b-content__col">
							<div class="b-input__wrap">
								<label class="b-input__label" for="sunrise-field-' . $field['id'] . '">' . $field['name'] . '</label>
								<input  type="text" value="' . get_option( $config['prefix'] . $field['id'] ) . '" name="sunrise[' . $field['id'] . ']" id="sunrise-field-' . $field['id'] . '" class="b-input" />
								<p class="b-input__description">' . $field['desc'] . '</p>
							</div>
		...
		public static function type_textarea( $field, $config ) {
			...
			return '<div class="b-content__row">
						<div class="b-content__col">
							<div class="b-input__wrap">
								<label class="b-input__label" for="sunrise-field-' . $field['id'] . '">' . $field['name'] . '</label>
								<textarea rows="1" name="sunrise[' . $field['id'] . ']" id="sunrise-field-' . $field['id'] . '" class="b-input b-input--textarea" >' . esc_textarea( stripslashes( get_option( $config['prefix'] . $field['id'] ) ) ) . '</textarea>
								<p class="b-input__description">' . $field['desc'] . '</p>
							</div>
						</div>
					</div>';
		...
		public static function type_checkbox( $field, $config ) {
			...
			return '<div class="b-content__row ' . $field['class'] . '">
						<div class="b-content__col">
							<div class="b-input__wrap b-input__wrap--checkbox">
							<span class="b-input__label">' . $field['label'] . '</span>
							<input class="b-checkbox__input" type="checkbox" name="sunrise[' . $field['id'] . ']" id="sunrise-field-' . $field['id'] . '"' . $checked . ' hidden/>
							<label class="b-checkbox__label" for="sunrise-field-' . $field['id'] . '">
							    <span class="b-checkbox__box"></span>' . $field['name'] . '
                            </label>
							<p class="b-input__description">' . $field['desc'] . '</p>
							</div>
						</div>
		...
		public static function type_select( $field, $config ) {
			...
			return '<div class="b-content__row">
						<div class="b-content__col">
							<label for="sunrise-field-' . $field['id'] . '">' . $field['name'] . '</label>
						</div>
						<div class="b-content__col">
							<select name="' . $name . '" class="widefat" id="sunrise-field-' . $field['id'] . '"' . $field['size'] . $field['multiple'] . '>' . implode( '',
					$options ) . '</select>
		...
		public static function type_radio( $field, $config ) {
			...
			if ( is_array( $field['options'] ) ) {
				foreach ( $field['options'] as $single ) {
					...
					$group[] = '<input class="b-radio__input" type="radio" name="sunrise[' . $field['id'] . ']" id="sunrise-field-' . $field['id'] . '-' . $single['value'] . '" value="' . $single['value'] . '"' . $checked . ' hidden />
					<label class="b-radio__label" for="sunrise-field-' . $field['id'] . '-' . $single['value'] . '">
					    <span class="b-radio__box"></span>' . $single['label'] . '
                    </label>';
				}
			}
			...
			return '<div class="b-content__row">
                        <div class="b-input__wrap">
                            <div class="b-input__label">' . $field['name'] . '</div>
                            <div class="b-radio__wrap">' . implode( '',
                        $group ) . '
                            </div>
                            <span class="b-input__description">' . $field['desc'] . '</span>
						</div>
					</div>';
		}
		...
		public static function type_number( $field, $config ) {
			...
			return '<div class="b-content__row">
						...
						<div class="b-content__col">
							<input type="number" value="' . get_option( $config['prefix'] . $field['id'] ) . '" name="sunrise[' . $field['id'] . ']" id="sunrise-field-' . $field['id'] . '" class="widefat" min="' . (string) $field['min'] . '" max="' . (string) $field['max'] . '" step="' . (string) $field['step'] . '" />
		...
		public static function type_media( $field, $config ) {
			...
			return '<div class="sunrise-media b-content__row">
						<div class="b-content__col">
							<div class="b-input__wrap b-input__wrap--media">
								<label class="b-input__label" for="sunrise-field-' . $field['id'] . '">' . $field['name'] . '</label>
								<input type="text" value="' . get_option( $config['prefix'] . $field['id'] ) . '" name="sunrise[' . $field['id'] . ']" id="sunrise-field-' . $field['id'] . '" class="b-input sunrise-media-value" /> 								<a href="javascript:;" class="b-button b-button--blue b-button--small sunrise-media-button hide-if-no-js">
									<img src="' . admin_url( 'images/media-button.png' ) . '" alt="" /> ' . __( 'Open media library',
						$config['textdomain'] ) . '</a>
								<p class="b-input__description">' . $field['desc'] . '</p>
							</div>
						</div>
		...
		public static function type_color( $field, $config ) {
			...
			return '<div class="b-content__row">
						...
						<div class="b-content__col">
							<div class="sunrise-color-picker">
								<input type="text" value="' . get_option( $config['prefix'] . $field['id'] ) . '" name="sunrise[' . $field['id'] . ']" id="sunrise-field-' . $field['id'] . '" class="regular-text sunrise-color-picker-value" />
								<span class="sunrise-color-picker-wheel"></span> 
								<a href="javascript:;" class="button sunrise-color-picker-button hide-if-no-js">
									<img alt="" src="data:image/png;base64," /> ' . __( 'Pick a color',
					$config['textdomain'] ) . '</a>
							</div>
							<span class="description">' . $field['desc'] . '</span>
						</div>
		...
		public static function type_checkbox_group( $field, $config ) {
			...
			return '<div class="b-content__row">
						<div class="b-content__col">' . $field['name'] . '</div>
						<div class="sunrise-checkbox-group b-content__col">' . implode( '',
					$group ) . '<span class="description">' . $field['desc'] . '</span></div>
					</div>';
		}
		...
		public static function type_tablerow( $field, $config ) {
			$field = wp_parse_args( $field, array(
				'name'     => __( 'Table row', $config['textdomain'] ),
				'content'  => '',
				'class'    => '',
				'filename' => '',
				'desc'     => ''
			...
			$file_path  = $home_path . '/' . $field['filename'];
			$file_class = file_exists( $file_path ) ? 'pw-exist ' : 'pw-noexist ';
			$row_class  = get_option( 'pushworld_protocol' ) == 'https' ? '' : ' pw-hidden ';

			return '<div class="' . $field['class'] . $row_class . ' b-content__row">
						<div class="b-content__col">' . $field['name'] . '</div>
                		<div class="' . $file_class . ' b-content__col">' . $field['content'] . '<br>
                			<span class="description">' . $field['desc'] . '</span>
                        </div>
                    </div>';
		}
		...
		public static function type_title( $field, $config ) {
			...
			return '<div class="b-content__row">
						<div class="b-content__col">
							<h3 class="sunrise-title-field">' . $field['name'] . '</h3>
						</div>
					</div>';
		}
		...
		public static function type_image_radio( $field, $config ) {
			...
			return '<div class="b-content__row">
						<div class="b-content__col">' . $field['name'] . '</div>
						<div class="b-content__col">
							<div class="sunrise-image-radio">' . implode( '',
					$options ) . '<input type="hidden" value="' . get_option( $config['prefix'] . $field['id'] ) . '" name="sunrise[' . $field['id'] . ']" id="sunrise-field-' . $field['id'] . '" /></div>
							<p class="description">' . $field['desc'] . '</p>
						</div>
					</div>';
		}
		...
		public static function type_size( $field, $config ) {
			...
			return '<div class="b-content__row">
						<div class="b-content__col">
							<label for="sunrise-field-' . $field['id'] . '-0">' . $field['name'] . '</label>
						</div>
						<div  class="b-content__col">
							<input type="number" value="' . $value[0] . '" name="sunrise[' . $field['id'] . '][0]" id="sunrise-field-' . $field['id'] . '-0" class="regular-text" min="' . (string) $field['min'] . '" max="' . (string) $field['max'] . '" step="' . (string) $field['step'] . '" /> 
							<select name="sunrise[' . $field['id'] . '][1]" id="sunrise-field-' . $field['id'] . '-1">' . str_replace( 'value="' . $value[1] . '"',
					'value="' . $value[1] . '" selected="selected"',
					implode( '', $units ) ) . '</select>
							<p class="description">' . $field['desc'] . '</p>
						</div>
					</div>';
		}
		...
		public static function options_page_tabs( $options, $config ) {
			...
			foreach ( (array) $options as $option ) {
				...
				if ( isset( $option['type'] ) && isset( $option['name'] ) && $option['type'] === 'opentab' ) {
					$tabs[] = '<span class="nav-tab b-tabs__label">' . $option['name'] . '</span>';
				}
			}
			...
			return ( count( $tabs ) ) ? '<div id="icon-options-general" class="icon32 hide-if-no-js"><br /></div><h2 id="sunrise-tabs" class="nav-tab-wrapper b-navbar b-navbar--main hide-if-no-js"><img class="b-logo" src="' . $pw_plugin_url . 'assets/imgs/logo-pw.png"><div class="b-tabs">' . implode( '',
					$tabs ) . '</div></h2>' : '';
		}
		...
		public static function options_page_notices( $options, $config ) {
			...
			$msgs = apply_filters( 'sunrise/page/notices', array(
				__( 'For full functionality of this page it is reccomended to enable javascript.',
					$config['textdomain'] ),
			...
			$output[] = self::notice( '<a href="http://enable-javascript.com/" target="_blank">' . $msgs[0] . '</a>.',
				'error hide-if-js' );
			// Show notice
			if ( $message !== 0 ) {
		...
		public static function options_page_panes( $options, $config ) {
			...
			foreach ( $options as $option ) {
				...
				elseif ( is_callable( array( $config['views_class'], 'type_' . $option['type'] ) ) ) {
					$panes[] = call_user_func( array( $config['views_class'], 'type_' . $option['type'] ), $option,
						$config );
				} // Show error message
				else {
					$panes[] = call_user_func( array( $config['views_class'], 'notice' ),
						'Sunrise: ' . sprintf( __( 'option type %s is not callable', $config['textdomain'] ),
							'<b>' . $option['type'] . '</b>' ), 'error' );
				}
			}
